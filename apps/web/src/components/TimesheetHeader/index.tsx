...
export const TimesheetHeader = (options: Options) => {
	const [projects] = useState<OptionItem[]>([
		emptyOptionItem,
		...buildProjects(options.projects),
	]);
	const days = getAllDaysInCurrentMonth(options.month);

	const getNbWorkingDays = (days: Days) =>
		days.reduce((acc, day) => (isWeekendDay(day.dayOfWeek) ? acc : acc + 1), 0);
		...
		<>
			...
			>
				...
				<span style={{ textAlign: 'center' }}>
					<Text size={'xl'} fw="bold" tt="uppercase">
						{getCurrentMonth(options.month)}
					</Text>
					<Text fs="italic">
						(working days: <strong>{getNbWorkingDays(days)}</strong>)
					</Text>
				</span>
				...
				{options.projectTasks && (
					<Group align={'flex-end'}>
						<NativeSelect
							data={[
								emptyOptionItem,
								...buildProjectTasks(options.projectTasks),
							]}
							label="Select task"
